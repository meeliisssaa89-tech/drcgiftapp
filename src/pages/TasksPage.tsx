...
export const TasksPage = () => {
  ...
  if (isLoading) {
    return <div className="flex flex-col items-center justify-center min-h-[50vh] gap-3">
        <Loader2 className="w-8 h-8 animate-spin text-primary" />
        <p className="text-muted-foreground">Loading tasks...</p>
      </div>;
  }
  return <div className="flex flex-col gap-4 pb-24 animate-fade-in">
      {/* Tab Navigation */}
      <div className="flex gap-2 overflow-x-auto scrollbar-hide -mx-1 px-1">
        {taskTabs.map(tab => <button key={tab.id} onClick={() => handleTabChange(tab.id)} className={cn("flex items-center gap-1.5 px-4 py-2 rounded-full font-medium whitespace-nowrap transition-all duration-200 text-sm active:scale-95", activeTab === tab.id ? "bg-primary text-primary-foreground shadow-lg shadow-primary/30" : "bg-secondary text-muted-foreground hover:bg-secondary/80")}>
      ...
      <div className="space-y-3">
        ...
        return <div key={task.id} className={cn("card-telegram transition-all duration-200 border-2 rounded-sm", status === 'claimable' && "ring-2 ring-primary/50 bg-primary/5", status === 'completed' && "opacity-60")}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0", status === 'completed' ? 'bg-green-500/20' : 'bg-secondary')}>
                    {status === 'completed' ? <CheckCircle className="w-5 h-5 text-green-500" /> : task.emoji}
                  </div>
                ...
                <div className="flex-shrink-0">
                  {status === 'waiting' && dailyTimer ? <div className="flex items-center gap-1 text-muted-foreground bg-secondary px-3 py-1.5 rounded-xl text-xs">
                      <Clock className="w-3.5 h-3.5" />
                      <span>{formatTimer(dailyTimer)}</span>
                    </div> : status === 'completed' ? <span className="text-xs text-green-500 bg-green-500/10 px-3 py-1.5 rounded-xl font-medium">
                      ✓ Done
                    </span> : <button onClick={() => handleTaskAction(task)} className={cn("px-4 py-1.5 rounded-xl font-medium text-sm flex items-center gap-1.5 transition-all duration-200 active:scale-95", status === 'claimable' ? "bg-green-500 text-white animate-pulse" : "bg-primary text-primary-foreground")}>
              ...
              <div className="flex items-center justify-between mt-3 gap-3">
                ...
                <div className="flex-1 flex items-center gap-2">
                  <ProgressBar progress={currentProgress} max={task.max_progress} showLabel={false} className="flex-1" />
                ...
                {timer !== undefined && timer > 0 && status === 'in_progress' && <div className="flex items-center gap-1 text-muted-foreground text-xs">
      ...
      {/* Empty state */}
      {filteredTasks.length === 0 && <div className="card-telegram min-h-[180px] flex flex-col items-center justify-center">
