...
import { Wallet, ArrowDown, ExternalLink, Loader2, Check, Copy, Gem } from 'lucide-react';
...
export const DepositPage = () => {
  ...
  const [activeMethod, setActiveMethod] = useState<'web3' | 'ton'>('ton');
  ...
  const openTonWallet = () => {
    buttonPress();
    // Check if Telegram WebApp is available and use native TON wallet
    if (window.Telegram?.WebApp) {
      // Try to open TON wallet through Telegram
    ...
    <div className="flex flex-col gap-6 pb-24 animate-fade-in">
      ...
      <div className="flex gap-2 p-1 bg-secondary rounded-xl">
        <button
          ...
          onClick={() => { buttonPress(); setActiveMethod('ton'); }}
        >
        ...
        <button
          ...
          onClick={() => { buttonPress(); setActiveMethod('web3'); }}
        >
      ...
      {activeMethod === 'ton' && (
        <div className="space-y-4">
          {/* TON Info Card */}
          <div className="card-telegram bg-gradient-to-br from-blue-500/10 to-blue-600/10 border-blue-500/30">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shadow-lg">
                <Gem className="w-8 h-8 text-white" />
              </div>
              <div>
                <h3 className="font-bold text-lg">Telegram TON Wallet</h3>
                <p className="text-sm text-muted-foreground">Fast & secure deposits</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-sm">
                <Check className="w-4 h-4 text-green-500" />
                <span>Native Telegram integration</span>
              </div>
            </div>

            <Button
              className="w-full h-14 text-lg mt-6 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700"
          ...
          {depositSettings.deposit_address && (
            <div className="card-telegram">
              <p className="text-sm text-muted-foreground mb-2">Send TON to this address:</p>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Exchange Rate: 1 TON = {currencySettings.exchange_rate} {currencySettings.name}
              </p>
            </div>
          )}
        </div>
